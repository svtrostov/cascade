...
//==================================================================
//Просмотр скриншота объекта доступа
function preview_irole(irole_id){
	var path = '/main/customcontent/irolescreenshot?custom=1&irole_id='+irole_id+'&unique_id='+String.uniqueID();
...
//==================================================================
//Генерация числового PIN кода
function generate_pin(count){
	count = count || 4;
	var result="";
	for(var i=0; i<count;i++) result+=String(Number.random(0, 9));
	return result;
};
...
function generate_password(plength){
	...
	var keylistspec="!@#_";
	var temp='';
	var len = Math.ceil(plength/2)-1;
	var ilen = Math.ceil(len/2)-1;
	var lenspec = Math.ceil(plength/4)-1;
	var blen = plength-lenspec-len-ilen;
	for (i=0;i<len;i++) temp+=keylistalpha.charAt(Math.floor(Math.random()*keylistalpha.length));
	for (i=0;i<blen;i++) temp+=String(keylistalpha.charAt(Math.floor(Math.random()*keylistalpha.length))).toUpperCase();
	for (i=0;i<lenspec;i++) temp+=keylistspec.charAt(Math.floor(Math.random()*keylistspec.length));
	for (i=0;i<ilen;i++) temp+=keylistint.charAt(Math.floor(Math.random()*keylistint.length));
