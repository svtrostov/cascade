...
var jsTable = new Class({
			...
			for(indexCol = 0; indexCol < this.columns.length; indexCol++){
				...
				if(cellDefaults['data']){
					switch(column['dataType']){
						...
						break;
						case 'num':
							cellDefaults['data'] = String(cellDefaults['data']).toFloat();
							td.set('text', Number(cellDefaults['data']).format(column['dataNumFormat']));
						break;
						default: td.set('text', cellDefaults['data']); break;
					}
				}

				if(cellDefaults['data']) td.set('html', cellDefaults['data']);
	...
	/*Сложный фильтр таблицы по нескольким условиям
		[{
			'column':'',			//Столбец для поиска
			'value':''				//Значение или массив значений
			'condition':'='			//Условие сравнения
		}],
		invert - инвертировать результаты
	 */
	multiFilter: function(conditions, invert){
		...
		var clearFilter = (typeOf(conditions)!='array'||!conditions.length);

		var tr, td, background, showing;
		var indx = 0;
		var is_section, condition;

		if(!clearFilter){
			//Просмотр условий
			for(var c=0; c<conditions.length; c++){
				condition = conditions[c];
				if(typeOf(conditions[c])!='object') continue;
				if(typeOf(conditions[c]['value'])!='array') conditions[c]['value'] = [conditions[c]['value']];
				if(!['=','!=','<>'].contains(conditions[c]['condition'])) conditions[c]['condition'] = '=';
			}//Просмотр условий
		}
		...
		for(var i=0; i<this.dataCells.length; ++i){
			...
			if(clearFilter){
				...
				for(var c=0; c<conditions.length; c++){
					...
					td = this.dataCells[i][condition['column']];
					str = String(td.retrieve('data')).toLowerCase();

					for(var j=0; j<condition['value'].length; ++j){
						if( str.indexOf(condition['value'][j]) > -1){
							found += ((condition['condition'] == '=') ? 1 : 0);
							break;
						}else{
							found += ((condition['condition'] == '=') ? 0 : 1);
						}
					}

				}//Просмотр условий
				showing = (conditions.length == found ? !invert : invert);
			}
