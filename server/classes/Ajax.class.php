<?php
/*==================================================================================================
Описание: Работа с AJAX ответами
...
class Ajax{
	 ...
	 */
	public function responseSuccess($title, $message){
		if($this->data['status'] != 'undefined' && $this->data['status'] != 'success') return false;
		$this->addMessage($title, $message, 'success', null, null);
		return true;
	}#end function
	...
	public function addRequired($file=null, $callback=null, $os=null, $browser=null){
		...
		foreach($file as $item){
			...
			$collection[] = array(
				'file'		=> $item[0],
				'callback'	=> (isset($item[1]) ? $item[1] : null),
				'os'		=> (isset($item[2]) ? $item[2] : null),
				'browser'	=> (isset($item[3]) ? $item[3] : null)
			);
		}
		$this->data['required'][] = $collection;
		return true;
	}#end function
