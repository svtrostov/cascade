...
class Main_PDFReportRequest extends FPDF{
	...
	function Header(){
		...
		if($this->page == 1){
			...
			$this->Cell(50, 4, '"УТВЕРЖДАЮ"', 0, 1, 'L');
			$this->Line(140,$y+15,200,$y+15);
			$this->SetXY(140,$y+16);
			$this->SetFont('Arial','',10);
			$this->Cell(50, 4, $this->report['request_info']['statement_employer'], 0, 1, 'L');
			$y+=30;
			$this->SetXY(140,$y);
			$this->SetFont('Arial','B',10);
			$this->Cell(50, 4, '"СОГЛАСОВАНО"', 0, 1, 'L');
			$this->Line(140,$y+15,200,$y+15);
			$this->SetXY(140,$y+16);
			$this->SetFont('Arial','',10);
			$this->Cell(50, 4, $this->report['request_info']['agreement_employer'], 0, 1, 'L');
			...
			$this->MultiCell(50,4,"Заявку сформировал:\n".$this->report['report_time']."\n".$this->report['report_user'],0,'R');
			*/
			$this->SetY(16);
			...
			$this->addAttrLine('',$this->report['request_info']['company_name'], 4);
			$this->addAttrLine('',$this->report['request_info']['post_name'], 4);
			if(!empty($this->report['request_info']['phone'])) $this->addAttrLine('',$this->report['request_info']['phone'], 4);
			$this->Ln(6);
	...
	function Footer(){
		...
		$this->SetFont('Arial','',10);
		$this->SetY(270);
		$this->Cell(100, 4, 'Заявитель:', 0, 1, 'L');
		$this->Line(10,285,65,285);
		$this->SetXY(10,285);
		$this->SetFont('Arial','',8);
		$this->Cell(55, 4, '(подпись)', 0, 1, 'C');
		$this->SetXY(65,280);
		$this->Cell(55, 4, $this->report['request_info']['employer_fio'], 0, 1, 'L');


		$this->SetFont('Arial','',10);
		$this->SetXY(120,270);
		$this->Cell(100, 4, 'Сотрудник ОКБ:', 0, 1, 'L');
		$this->Line(120,285,155,285);
		$this->Line(160,285,200,285);
		$this->SetXY(120,285);
		$this->SetFont('Arial','',8);
		$this->Cell(35, 4, '(подпись)', 0, 1, 'C');
		$this->SetXY(160,285);
		$this->Cell(40, 4, '(Ф.И.О.)', 0, 1, 'C');
	}#end function



	//Добавить строку ключ: значение
	public function addAttrLine($key, $value, $height=4, $key_width=25, $value_width=150){
