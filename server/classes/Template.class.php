<?php
/*==================================================================================================
Описание: Обработчик шаблонов страниц
	...
	*/
	...
	public function parseTemplate(){
		...
		if(!$this->internal_init){
			$this->variables['USER_LANGUAGE'] 	= User::_getLanguage();
			$this->variables['USER_THEME'] 		= User::_getTheme();
			$this->variables['REQUEST_MODULE'] 	= addslashes(Request::_get('module'));
			$this->variables['SESSION_COOKIE'] 	= strtoupper(Config::getOption('general','session_name','IAMCLIENT'));
		...
		#Но не более 10 раз
		do{
			$template = preg_replace_callback('/{\%(\S+)\%}/s', array($this,'parseTemplateCallback'), $template, -1, $count);
			$level++;
