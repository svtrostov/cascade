<?php
/*==================================================================================================
Описание: Основные функции инициализации приложения
Stanislav V. Tretyakov (svtrostov@yandex.ru)
==================================================================================================*/
if(!defined('APP_INSIDE')) die('Direct access not allowed!');


if(!defined('EVENT_PHP_ERROR')){
	define('EVENT_PHP_ERROR', 'EVENT_PHP_ERROR');
}
...
function loader_init(){
	...
	if(!defined('APP_LANGUAGE')) define('APP_LANGUAGE', strtolower(Config::getOption('general','default_language','ru')));
...
function loader_loadConfig(){
	...
	if(empty($modules)) return debugError(array(
		'id'		=> 'ELOAD001',
		'desc'		=> 'Не заданы настройки модулей в config/modules.config.php',
		'data'		=> $modules,
		'return'	=> false,
		'file'		=> __FILE__,
		'line'		=> __LINE__,
...
function loader_initDatabases(){
	...
	$dbs = Config::getOption('general','databases',false);
	...
	foreach($dbs as $name=>$options){
		...
		if(empty($options)||!is_array($options)||empty($options['host'])||empty($options['username'])||empty($options['database'])){
			return debugError(array(
				'id'		=> 'ELOAD003',
				'desc'		=> 'Некоректно заданы настройки соединения с базой данных в config/general.config.php, секция databases',
