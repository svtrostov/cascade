...
switch($request_action){
	...
	case 'irole.edit':
		...
		if(empty($iresource_id)||empty($irole_id)||empty($full_name)) return Ajax::_responseError('Ошибка выполнения','Некорректный запрос');
	...
	case 'irole.screenshot.upload':
		...
		$admin_iresource = new Admin_IResource();
		if(!$admin_iresource->iroleExists($iresource_id, $irole_id)) return Ajax::_responseError('Ошибка выполнения','Объекта доступа в указанном информационном ресурсе не существует');
	...
	case 'iresource.import':
		...
		if(empty($iresource_id)||empty($import_from)||empty($import_type)) return Ajax::_responseError('Ошибка выполнения','Некорректный запрос');

		$admin_iresource = new Admin_IResource();
		if(!$admin_iresource->iresourceExists($iresource_id)) return Ajax::_responseError('Ошибка выполнения','Информационного ресурса ID:'.$iresource_id.' не существует');
