...
switch($report_type){
	...
	#Печатная форма заявки для исполнителей
	case 'performer':
		$report_data['request_id'] = $request->getId('request_id',0);
		$report_data['iresource_id'] = $request->getId('iresource_id',0);
		if(empty($report_data['request_id'])||empty($report_data['iresource_id'])) die('Incorrect request_id/iresource_id param');
		goto LABEL_REPORT_PERFORMER;
	break;

	#Печатная форма заявки для наблюдателей
	case 'watcher':
		$report_data['request_id'] = $request->getId('request_id',0);
		$report_data['iresource_id'] = $request->getId('iresource_id',0);
		if(empty($report_data['request_id'])||empty($report_data['iresource_id'])) die('Incorrect request_id/iresource_id param');
		goto LABEL_REPORT_WATCHER;
	break;
...
/***********************************************************************
 * ОБРАБОТЧИКИ: Печатная форма заявки для исполнителей
 **********************************************************************/
LABEL_REPORT_PERFORMER:

	$main_request = new Main_Request();
	$main_employer = new Main_Employer();

	$gk_request = $main_employer->getActiveRequests(array(
		'request_id'	=> array($report_data['request_id']),
		'iresource_id'	=> array($report_data['iresource_id']),
		'roles'			=> array(3),
		'single'		=> true
	));

	if(empty($gk_request) || $gk_request['gatekeeper_role']!=3 || !$main_employer->canApprove($report_data['request_id'], $report_data['iresource_id'])){
		die('You are not allowed to view this content: gk');
	}
	...
	for($i=0;$i<$count;$i++){
		...
		if($report_data['request_info']['request_type']==3){
			$iroles_raw[$i]['ir_selected_name'] = 'Блокировать';
		}else{
	...
	$report_data['request_info']['employer_fio'] = $main_employer->getEmployerName($report_data['request_info']['employer_id'],'{last} {f}.{m}.');


	#Рендеринг заявки в PDF
	$pdf = new Main_PDFReportPerformer($report_data);
...
/***********************************************************************
 * ОБРАБОТЧИКИ: Печатная форма заявки
 **********************************************************************/
...
/***********************************************************************
 * ОБРАБОТЧИКИ: Печатная форма заявки для наблюдателей
 **********************************************************************/
LABEL_REPORT_WATCHER:

	$main_request = new Main_Request();
	$main_employer = new Main_Employer();

	if(!$main_employer->dbCanWatchRequest(0, $report_data['request_id'], $report_data['iresource_id'])){
		die('You are not allowed to view this content: watcher');
	}
